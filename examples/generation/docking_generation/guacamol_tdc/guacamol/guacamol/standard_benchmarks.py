...
def isomers_c9h10n2o2pf2cl(mean_function="geometric",
                           n_samples=250) -> GoalDirectedBenchmark:
    """
...
def similarity_cns_mpo(smiles,
                       molecule_name,
                       max_logP=5.0) -> GoalDirectedBenchmark:
    ...
    anti_fp = TanimotoScoringFunction(smiles,
                                      fp_type="ECFP6",
                                      score_modifier=MinGaussianModifier(
                                          mu=0.70, sigma=0.1))
...
def similarity(
    ...
    scoring_function = TanimotoScoringFunction(target=smiles,
                                               fp_type=fp_type,
                                               score_modifier=modifier)
    if rediscovery:
...
def logP_benchmark(target: float) -> GoalDirectedBenchmark:
    ...
    objective = RdkitScoringFunction(descriptor=logP,
                                     score_modifier=GaussianModifier(mu=target,
                                                                     sigma=1))
...
def novelty_benchmark(training_set_file: str,
                      number_samples: int) -> DistributionLearningBenchmark:
    smiles_list = [s.strip() for s in open(training_set_file).readlines()]
    return NoveltyBenchmark(number_samples=number_samples,
                            training_set=smiles_list)


def kldiv_benchmark(training_set_file: str,
                    number_samples: int) -> DistributionLearningBenchmark:
    smiles_list = [s.strip() for s in open(training_set_file).readlines()]
    return KLDivBenchmark(number_samples=number_samples,
                          training_set=smiles_list)
...
def amlodipine_rings() -> GoalDirectedBenchmark:
    ...
    rings = RdkitScoringFunction(descriptor=num_rings,
                                 score_modifier=GaussianModifier(mu=3,
                                                                 sigma=0.5))
...
def smarts_with_other_target(smarts: str,
                             other_molecule: str) -> ScoringFunction:
    ...
    bz = RdkitScoringFunction(descriptor=bertz,
                              score_modifier=GaussianModifier(mu=target_bertz,
                                                              sigma=30))
...
def valsartan_smarts() -> GoalDirectedBenchmark:
    ...
    return GoalDirectedBenchmark(
        ...
        objective=smarts_with_other_target(valsartan_smarts,
                                           sitagliptin_smiles),
        contribution_specification=specification,
    )
...
def pioglitazone_mpo() -> GoalDirectedBenchmark:
    ...
    mw = RdkitScoringFunction(descriptor=mol_weight,
                              score_modifier=GaussianModifier(mu=target_molw,
                                                              sigma=10))
