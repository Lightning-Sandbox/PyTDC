...
class BasicEditor(Editor):
    ...
    def predict_del(self, g, h_node):
        ...
        h_edge = torch.cat([h_u, h_edge, h_v],
                           dim=1)  # (n_edges, n_node_hidden*2+n_edge_hidden)
    ...
    def loss(self, batch, metrics=["loss"]):
        ...
        ### losses
        loss_del = F.cross_entropy(pred_del, targ_del,
                                   reduction="none")  # (tot_n_edge,)
        loss_add = F.cross_entropy(pred_add, targ_add,
                                   reduction="none")  # (tot_n_node,)
        loss_arm = F.cross_entropy(pred_arm, targ_arm,
                                   reduction="none")  # (tot_n_node,)
