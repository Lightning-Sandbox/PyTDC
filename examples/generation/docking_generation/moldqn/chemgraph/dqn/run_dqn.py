...
def run_training(hparams, environment, dqn):
    ...
    with tf.Session() as sess:
        ...
        exploration = schedules.PiecewiseSchedule(
            [
                (0, 1.0),
                (int(hparams.num_episodes / 2), 0.1),
        ...
        for episode in range(hparams.num_episodes):
            ...
            if (
                    episode + 1
            ) % hparams.save_frequency == 0:  ### save_frequency=200 * step=40
...
def _episode(
    ...
    for step in range(hparams.max_steps_per_episode):
        ...
        ### logging
        print("step " + str(step) + " SMILES: " + result.state)
        if step == hparams.max_steps_per_episode - 1:  ## last step
...
def _step(environment, dqn, memory, episode, hparams, exploration, head):
    ...
    ### select 1 action from valid_actions                  epsilon vs 1-epsilon
