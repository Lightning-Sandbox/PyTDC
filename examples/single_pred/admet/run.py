...
parser.add_argument('-m',
                    ...
                    default='RDKit2D',
                    type=str,
                    help='model name. select from CNN, RDKit2D, and Morgan.')

args = parser.parse_args()
drug_encoding = args.model

if drug_encoding not in [
        'RDKit2D', 'Morgan', 'CNN', 'NeuralFP', 'MPNN', 'AttentiveFP',
...
for seed in [1, 2, 3, 4, 5]:
    ...
    for benchmark in group:
        train, valid = group.get_train_valid_split(benchmark=benchmark['name'],
                                                   split_type='default',
                                                   seed=seed)

        train = data_process(X_drug=train.Drug.values,
                             y=train.Y.values,
                             drug_encoding=drug_encoding,
                             split_method='no_split')

        val = data_process(X_drug=valid.Drug.values,
                           y=valid.Y.values,
                           drug_encoding=drug_encoding,
                           split_method='no_split')

        test = data_process(X_drug=benchmark['test'].Drug.values,
                            y=benchmark['test'].Y.values,
                            drug_encoding=drug_encoding,
                            split_method='no_split')
