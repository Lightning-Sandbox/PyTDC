...
pert_data.load(data_path='./data/' +
               dataset.lower())  # specific dataset name and adata object

train_not_seen = [
    i for i in set2conditions['train']
    if not filter_pert_in_go(i, pert_data.pert_names)
]
val_not_seen = [
    i for i in set2conditions['val']
    if not filter_pert_in_go(i, pert_data.pert_names)
]
test_not_seen = [
    i for i in set2conditions['test']
    if not filter_pert_in_go(i, pert_data.pert_names)
]
