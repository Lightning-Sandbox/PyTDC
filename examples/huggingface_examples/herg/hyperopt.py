...
def prepare_data(drug_encoding):
    return dp_utils.data_process(X_drug=X,
                                 y=y,
...
def train_fn(hparams, optimal=False):
    ...
    if drug_encoding == "Morgan":
        config = dp_utils.generate_config(
            ...
            mlp_hidden_dims_drug=[int(hparams["mlp_hidden_dims_drug"])] *
            int(hparams["mlp_num_layers_drug"]))
        ...
        config = dp_utils.generate_config(
            ...
            batch_size=int(hparams["batch_size"]),
            cnn_drug_filters=[
                filter_base_size, filter_base_size * 2, filter_base_size * 3
            ],
            cnn_drug_kernels=[
                kernel_base_size, kernel_base_size * 2, kernel_base_size * 3
            ])
        ...
        config = dp_utils.generate_config(
            ...
            neuralfp_predictor_hid_dim=int(
                hparams["neuralfp_predictor_hid_dim"]))
...
if drug_encoding == "Morgan":
    ...
    search_space.update({
        "gnn_hid_dim_drug":
            tune.choice([8, 16, 32, 64, 128, 256, 512]),
        "gnn_num_layers":
            tune.choice([1, 2, 3, 4]),
        "neuralfp_max_degree":
            tune.choice([5, 10, 20]),
...
tuner = tune.Tuner(
    tune.with_resources(tune.with_parameters(train_fn), resources={"gpu": 1}),
...
optimal_hparam = {
    i[7:] if i[:7] == 'config/' else i: j for i, j in optimal_hparam.items()
}
